...
def take_inputs() -> tuple:
    ...
    Also creates parameters_entered file
    '''
    print("Rename the gtfs.zip to network_gtfs.zip and place it in main directory."
          " For example, for {NETWORK_NAME}, place the {NETWORK_NAME}_gtfs.zip in the main directory.")
    print("Enter Example parameters to build test case.")
    

    NETWORK_NAME = input("Enter Network name in small case. Example: {NETWORK_NAME}\n: ")
    ...
    if not os.path.exists(f'./Data/GTFS/{NETWORK_NAME}/gtfs_o'):
            os.makedirs(f'./Data/GTFS/{NETWORK_NAME}/gtfs_o')
   
    READ_PATH = f'./Data/GTFS/{NETWORK_NAME}/gtfs_o'
    SAVE_PATH = f'./Data/GTFS/{NETWORK_NAME}/'
    #Generating patrameters file
    param_list = [BUILD_TRANSFER, NETWORK_NAME, BUILD_TBTR_FILES, 1, 1]
...
def read_gtfs(READ_PATH: str, NETWORK_NAME: str):
    ...
    root.withdraw()
    print("Select gtfs zip file in pop up window")
    # Open file explorer with filter for .zip files
    ...
    try:
        ...
        print("calendar_dates.txt missing")
        print("creating calendar_dates.txt")
        os.system(f'python calendar_to_calendar_dates.py')
        try:
            calendar_dates = pd.read_csv(f'{READ_PATH}/calendar_dates.txt')
        except FileNotFoundError:
            print("Failed to open calander")
    try:
