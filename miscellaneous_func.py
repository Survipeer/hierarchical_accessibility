...
def print_query_parameters(NETWORK_NAME: str, SOURCE: int, D_TIME, MAX_TRANSFER: int, WALKING_FROM_SOURCE: int, variant: int, no_of_partitions=None, weighting_scheme=None, partitioning_algorithm=None) -> None:
    """
    ...
    Examples:
        >>> overlap = print_query_parameters('{NETWORK_NAME}', 36, 52, pd.to_datetime('2020-09-14 20:00:00'), 4, 1, 0, None, None, None)
...
def get_full_trans(NETWORK_NAME: str, time_limit) -> None:
    ...
    Examples:
        >>> get_full_trans('{NETWORK_NAME}', 180)
    '''
    #    print('editing transfers')
    transfers_file = pd.read_csv(f'Data/GTFS/{NETWORK_NAME}/transfers.txt', sep=',')
...
def get_random_od(routes_by_stop_dict: dict, NETWORK_NAME: str) -> None:
    ...
    Examples:
        >>> get_random_od(routes_by_stop_dict, '{NETWORK_NAME}')
    ...
    random_od_db.iloc[:desired_len].to_csv(f"./GTFS/{NETWORK_NAME}_randomOD.csv", index=False)
...
def load_TE_graph(NETWORK_NAME: str, stop_times_file)-> tuple:
    ...
    """
    try:
        with open(f"./Data/time_expanded/{NETWORK_NAME}/graph_{NETWORK_NAME}", 'rb') as file:
            G = pickle.load(file)
