...
def find_transfer_len(source_info, G, stops_list, WALKING_LIMIT):
    """
    ...
    Returns:
        temp_list (list): list of format: [(bus stop id, osm node id, walking time in seconds)]

    Examples:
        >>> temp_list = find_transfer_len(source_info)
    """
    WALKING_SPEED = 1.2  # meters/second (4.3 km/h) - IRC 103 standard for India
    out = nx.single_source_dijkstra_path_length(G, source_info[1], cutoff=WALKING_LIMIT * 2 * WALKING_SPEED, weight='length')
    reachable_osmnodes = set(out.keys())
    temp_list = [(source_info[0], stopid, round(out[osm_nodet] / WALKING_SPEED, 1)) for (stopid, osm_nodet) in stops_list if osm_nodet in reachable_osmnodes]
    return temp_list

def transitive_closure(input_list, G_new):
    """
...
def post_process(G_new, NETWORK_NAME: str, ini_len: int, breaker: str, start_time: float) -> None:
    """
...
def initialize():
    ...
    return breaker, CORES, WALKING_LIMIT, start_time, USE_PARALlEL, GENERATE_LOGFILE
...
if __name__ == '__main__':
    ...
    BUILD_TRANSFER, NETWORK_NAME, *_ = parameter_files
    if BUILD_TRANSFER == 1:
        ...
        breaker, CORES, WALKING_LIMIT, start_time, USE_PARALlEL, GENERATE_LOGFILE = initialize()
        start_time = time()
        if GENERATE_LOGFILE == 1:
            sys.stdout = open(f'./logs/Transfer_builder_{NETWORK_NAME}', 'w')

        G, stops_list = extract_graph(NETWORK_NAME, breaker)

        find_func = partial(find_transfer_len, G=G, stops_list=stops_list, WALKING_LIMIT=WALKING_LIMIT)

        if USE_PARALlEL != 0:
            with Pool(CORES) as pool:
                result = pool.map(find_func, stops_list)
        else:
            result = [find_func(source_info) for source_info in tqdm(stops_list)]
        print(breaker)
        ...
        #trans_func = partial(transitive_closure, G_new=G_new)
        #if USE_PARALlEL != 0:
            #print("Ensuring Transitive closure in parallel...")
            #with Pool(CORES) as pool:
                #new_edge_list = pool.map(trans_func, connected_compnent_list)
        #else:
            #print("Ensuring Transitive closure in serial...")
            #new_edge_list = [trans_func(input_list) for input_list in tqdm(connected_compnent_list)]

        #new_edge_list = [y for x in new_edge_list for y in x]
        #G_new.add_weighted_edges_from(new_edge_list)
        #post_process(G_new, NETWORK_NAME, ini_len, breaker, start_time)
    ...
    print(f"\nTotal script run time: {round((time() - start_time)/60, 2)} minutes", file=sys.__stdout__)
